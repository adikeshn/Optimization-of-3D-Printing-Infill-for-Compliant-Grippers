from experiments.simulations.convert_step_gmsh import convert_to_mesh
from experiments.simulations.calc_gripper_metrics import load_Domain_sfepy, generate_regions, calc_gripper_results
from experiments.simulations.util import plotPoints, computePseudoCGS, calc_force_area, plotDisplacement, minmax, fusionAccuracy
from experiments.simulations.infill_generation_pt2 import get_grid_infill, get_honeycomb_infill, get_triangle_infill, get_finray_infill
...
def model_cad(step_path, mesh_size, plot = False):
    ...
    top_5_20_band = von_mises[(von_mises >= p80) & (von_mises <= p95)]
    return [np.mean(top_5_20_band), max(np.linalg.norm(disp, axis=1))]
    


def get_metrics(job, part, infill_type, den, mesh_size, infill_thickness, plt = False):
...
def main():
    ...
    for key in densities:
        for den_val in range(len(densities[key])):
            ...
            mets, density = get_metrics(part, key, densities[key][den_val], 0.25, 0.45, plt=False)
...
def vain():
    print(model_cad('finr36', 0.25, plot = False))
vain()
